...
cp $HOME/bin/glide .
docker run -v "${SRC_PATH}:/go/src/$PROJECT_NAME" -w "/go/src/$PROJECT_NAME" -e GO15VENDOREXPERIMENT=1 golang:1.5 /bin/bash -c "./glide up && CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -v"
